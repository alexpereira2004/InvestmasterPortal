    ...
    public CarteiraDto convert(List<Object> row) {
        ...
        if (tamanhosValidos.contains(row.size())) {
            return CarteiraDto.builder()
                    ...
                    .precoAtual(StringParser.toBigDecimal(row.get(3).toString()))
                    .valorizacao(StringParser.toBigDecimal(row.get(4).toString()))
                    .quantidade(StringParser.toInteger(row.get(5).toString()))
                    .totalInvestido(StringParser.toBigDecimal(row.get(6).toString()))
                    .atualizacao(StringParser.toBigDecimal(row.get(7).toString()))
                    .resultado(StringParser.toBigDecimal(row.get(8).toString()))
